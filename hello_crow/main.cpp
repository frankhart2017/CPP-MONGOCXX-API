...
int main(int argc, char *argv[]) {
  ...
  auto collection = conn["heroku_2nxcrj5t"]["contacts"];
  ...
  CROW_ROUTE(app, "/contact/<string>/<string>")
    ([&collection](const request &req, response &res, string name, string email){
      auto doc = collection.find_one(
        make_document(kvp("name", name), kvp("email", email)));
      if(!doc) {
  ...
  CROW_ROUTE(app, "/addContact/<string>/<string>").methods(HTTPMethod::Post)
    ([&collection](const request &req, response &res, string name, string email){
      ...
      res.set_header("Content-Type", "text/plain");
      res.write("Document Inserted Successfully!");
      res.end();
    });
